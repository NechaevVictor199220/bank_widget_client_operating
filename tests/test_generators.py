...
def test_filter_by_currency():
    ...
    jpy_transactions = list(filter_by_currency(test_transactions, 'JPY'))
    assert len(jpy_transactions) == 0


def test_transaction_descriptions():
    """Тесты для генератора описаний транзакций"""
    ...
    empty_data = [{"description": ""}, {"description": "Не пустое"}]
    gen = transaction_descriptions(empty_data)
    assert next(gen) == ""
    assert next(gen) == "Не пустое"